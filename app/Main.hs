...
hypothesis xs' w b = fmap (+ b) (pure (*) <*> w <*> xs')
...
updateWeights xs hs ys w lr = fmap (-(lr * dw)) w
  where
    dw = (2 * dot xs (pure (-) <*> ys <*> hs)) / fromIntegral (length xs) -- number of training examples. might to change it up in a 2d array?

-- dW = - ( 2 * ( self.X.T ).dot( self.Y - Y_pred )  ) / self.m
updateBias :: [Float] -> [Float] -> [Float] -> Float -> Float -> Float
updateBias xs hs ys b lr = b - (lr * db)
  where
    db = 2 * sum (pure (-) <*> ys <*> hs) / fromIntegral length xs
